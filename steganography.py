...
"""simple cli for hiding and revealing images inside images it is basically a copy pasta wrapper around
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(prog='steganography.py')
    ap.add_argument("-i", "--input", type=str, nargs='?',
                    required=True, const=True, default=None,
                    help="path to the visible image to perform steganography on")
    ap.add_argument("-e", "--encode", type=str2bool, nargs='?',
                    required=False, const=True, default=False,
                    help="encode one image into another image, if true secondary must also be set")
    ap.add_argument("-s", "--secondary", type=str, nargs='?',
    ...
    if args["encode"] and args["secondary"] is None:
        print("missing secondary image path")
        ap.print_help()

    if args["encode"]:
        merge(args["input"], args["secondary"], args["output"])
    else:
        unmerge(args["input"], args["output"])
